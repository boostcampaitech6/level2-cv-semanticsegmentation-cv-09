...
model_urls = {
    ...
    "swinv2_base_window16_256": "https://github.com/SwinTransformer/storage/releases/download/v2.0.0/swinv2_base_patch4_window12to16_192to256_22kto1k_ft.pth",
}
...
def swin_v2(size, img_size=256, in_22k=False, **kwargs):
    ...
    if size == "swinv2_tiny_window16_256":
        model = SwinTransformerV2(
            img_size=img_size, window_size=16, embed_dim=96,
            depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24], **kwargs)
        request.urlretrieve(model_urls[size], filename=f'./checkpoints/{size}.pth')
        checkpoint = torch.load(f'./checkpoints/{size}.pth')["model"]
        ...
        model = SwinTransformerV2(
            img_size=img_size, window_size=8, embed_dim=96,
        ...
        model = SwinTransformerV2(
            img_size=img_size, window_size=16, embed_dim=128,
